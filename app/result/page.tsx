          ...
          <div style={{ flex: 1, overflowY: 'auto', scrollbarWidth: 'thin' }}>
            ...
            {activeTab === 'verdict' && (
              <div>
                ...
                {/* Stats row */}
                {(() => {
                  const items = result.roastItems || [];
                  const crit = items.filter(x => x.severity === 'critical').length;
                  const warn = items.filter(x => x.severity === 'warning').length;
                  const note = items.filter(x => x.severity === 'note').length;
                  const total = items.length;
                  return total > 0 ? (
                    <div style={{ margin: '0 14px 10px', padding: '10px 12px', background: 'var(--bg)', border: '1px solid var(--border)', display: 'flex', flexDirection: 'column', gap: 6 }}>
                      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                        <span style={{ fontSize: 9, letterSpacing: '0.12em', color: 'var(--text-3)' }}>FINDINGS BREAKDOWN</span>
                        <span style={{ fontSize: 9, color: 'var(--text-3)' }}>{total} total</span>
                      </div>
                      {[
                        { label: '💀 CRITICAL', count: crit, color: 'var(--red)', bg: 'rgba(192,83,78,0.18)' },
                        { label: '⚠ WARNING',  count: warn, color: 'var(--orange)', bg: 'rgba(234,140,30,0.18)' },
                        { label: '📝 NOTE',     count: note, color: 'var(--green)',  bg: 'rgba(90,158,111,0.18)' },
                      ].map(({ label, count, color, bg }) => count > 0 && (
                        <div key={label} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                          <span style={{ fontSize: 8.5, color, width: 72, letterSpacing: '0.06em', flexShrink: 0 }}>{label}</span>
                          <div style={{ flex: 1, height: 4, background: 'var(--border)', borderRadius: 2, overflow: 'hidden' }}>
                            <div style={{ width: `${(count / total) * 100}%`, height: '100%', background: color, borderRadius: 2, transition: 'width 0.8s cubic-bezier(0.4,0,0.2,1)' }} />
                          </div>
                          <span style={{ fontSize: 8.5, color, width: 14, textAlign: 'right', flexShrink: 0 }}>{count}</span>
                        </div>
                      ))}
                    </div>
                  ) : null;
